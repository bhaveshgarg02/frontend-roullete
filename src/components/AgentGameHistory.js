    ...
    <LayoutAgent>
      <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
        ...
        {games.length === 0 ? (
          <p>
            {selectedDate
              ? `No games found for the selected date: ${moment(selectedDate).tz("Asia/Kolkata").format("YYYY-MM-DD")}`
              : 'No games available.'}
          </p>
        ) : (
          <>
              ...
              <tbody>
                {games.map((game) => (
                  <tr key={game.gameNumber}>
                    ...
                    <td>{moment(game.date).tz("Asia/Kolkata").format("YYYY-MM-DD HH:mm:ss")}</td>
                    {[...Array(10).keys()].map((number) => {
